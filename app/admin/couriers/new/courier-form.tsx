...
export function CourierForm({ initialData, action, submitLabel, existingOperators = [] }: CourierFormProps) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-full items-stretch pt-4">
      {/* Columna del Formulario (NARANJA) */}
      <form action={formAction} className="flex flex-col gap-4 w-full max-w-xl border-2 border-brand-safety p-6 relative bg-black">
        {initialData?.id && <input type="hidden" name="id" value={initialData.id} />}
        <div className="absolute top-0 left-0 bg-brand-safety text-black font-sans font-bold uppercase px-4 py-1 -translate-y-1/2 translate-x-4 text-xs z-10">
      ...
      <div className="flex-1 border-2 border-brand-neon bg-black p-6 flex flex-col gap-4 relative">
        <div className="absolute top-0 left-0 bg-brand-neon text-black font-sans font-bold uppercase px-4 py-1 -translate-y-1/2 translate-x-4 text-xs z-10">
